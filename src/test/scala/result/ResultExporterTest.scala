...
class ResultExporterTest extends FunSuite:
  ...
  test("Result export must succeed"):
    ...
    val out = sut.doExport(Vector.empty, Vector.empty, Vector.empty, Vector.empty, Vector.empty)
    assert(out.isRight)
  ...
  test("Result export must contain expected json results in knowl results"):
    ...
    val out = sut.doExport(knowlResults, Vector.empty, Vector.empty, Vector.empty, Vector.empty)
    assert(out.isRight)
  ...
  test("Result export must contain expected json results in note results"):
    ...
    val out = sut.doExport(List.empty, Vector.empty, Vector(expectedNotesResult), Vector.empty, src)
    assert(out.isRight)
